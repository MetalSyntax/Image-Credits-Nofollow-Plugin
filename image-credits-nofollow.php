...
class ImageCreditsNofollowPlugin {
	...
	function add_fields($form_fields, $post) {
		//New Element
		$form_fields['credits_ID'] = array(
			'label' => __( 'ID', 'image-credits-nofollow' ),
			'input' => 'text',
			'value' => get_post_meta($post->ID, '_wp_attachment_source_ID', true) /*,
			'helps' => __( 'Source name of the image.', 'image-credits-nofollow' )*/
		);

		$form_fields['credits_Autor'] = array(
			'label' => __( 'Autor', 'image-credits-nofollow' ),
			'input' => 'text',
			'value' => get_post_meta($post->ID, '_wp_attachment_source_Autor', true) /*,
			'helps' => __( 'Source name of the image.', 'image-credits-nofollow' )*/
		);

		$form_fields['credits_Publisher'] = array(
			'label' => __( 'Publisher', 'image-credits-nofollow' ),
			'input' => 'text',
			'value' => get_post_meta($post->ID, '_wp_attachment_source_Publisher', true) /*,
			'helps' => __( 'Source name of the image.', 'image-credits-nofollow' )*/
		);

		$form_fields['credits_License'] = array(
			'label' => __( 'License', 'image-credits-nofollow' ),
			'input' => 'text',
			'value' => get_post_meta($post->ID, '_wp_attachment_source_License', true) /*,
			'helps' => __( 'Source name of the image.', 'image-credits-nofollow' )*/
		);
	...
	function save_fields($post, $attachment) {
		//New
		if (isset($attachment['credits_ID'])) {
			$credits_ID = get_post_meta($post['ID'], '_wp_attachment_source_ID', true);

			if ($credits_ID != esc_attr($attachment['credits_ID'])) {
				if (empty($attachment['credits_ID'])) {
					delete_post_meta($post['ID'], '_wp_attachment_source_ID');
				} else {
					update_post_meta($post['ID'], '_wp_attachment_source_ID', esc_attr($attachment['credits_ID']));
				}
			}
		}

		if (isset($attachment['credits_Autor'])) {
			$credits_Autor = get_post_meta($post['ID'], '_wp_attachment_source_Autor', true);

			if ($credits_Autor != esc_attr($attachment['credits_Autor'])) {
				if (empty($attachment['credits_Autor'])) {
					delete_post_meta($post['ID'], '_wp_attachment_source_Autor');
				} else {
					update_post_meta($post['ID'], '_wp_attachment_source_Autor', esc_attr($attachment['credits_Autor']));
				}
			}
		}
		if (isset($attachment['credits_Publisher'])) {
			$credits_Publisher = get_post_meta($post['ID'], '_wp_attachment_source_Publisher', true);

			if ($credits_Publisher != esc_attr($attachment['credits_Publisher'])) {
				if (empty($attachment['credits_Publisher'])) {
					delete_post_meta($post['ID'], '_wp_attachment_source_Publisher');
				} else {
					update_post_meta($post['ID'], '_wp_attachment_source_Publisher', esc_attr($attachment['credits_Publisher']));
				}
			}
		}
		if (isset($attachment['credits_License'])) {
			$credits_License = get_post_meta($post['ID'], '_wp_attachment_source_License', true);

			if ($credits_License != esc_attr($attachment['credits_License'])) {
				if (empty($attachment['credits_License'])) {
					delete_post_meta($post['ID'], '_wp_attachment_source_License');
				} else {
					update_post_meta($post['ID'], '_wp_attachment_source_License', esc_attr($attachment['credits_License']));
				}
			}
		}
		//Old
	...
	function get_image_credits() {
		...
		foreach ($attachment_ids as $id) {
			$credit_ID = esc_attr(get_post_meta($id, '_wp_attachment_source_ID', true));
			$credit_Autor = esc_attr(get_post_meta($id, '_wp_attachment_source_Autor', true));
			$credit_Publisher = esc_attr(get_post_meta($id, '_wp_attachment_source_Publisher', true));
			$credit_License = esc_attr(get_post_meta($id, '_wp_attachment_source_License', true));
			...
			if (!empty($credit_source)) {
				if (empty($credit_link)) {
					$credits[] = "ID: " . $credit_ID ." - Autor: ". $credit_Autor ." - Fuente: ". $credit_source ." - Publicado: ". $credit_Publisher ." - Licencia: ". $credit_License;
				} else {
